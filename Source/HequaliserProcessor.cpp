...
juce::AudioProcessorValueTreeState::ParameterLayout createParameterLayout()
{
    ...
    for (size_t i = 0; i < defaults.size(); ++i)
    {
        ...
        auto typeParameter = std::make_unique<juce::AudioParameterChoice> (juce::ParameterID{FrequalizerAudioProcessor::getTypeParamName (i),1},
                                                                     prefix + TRANS ("Filter Type"),
                                                                     FrequalizerAudioProcessor::getFilterTypeNames(),
                                                                     defaults [i].type);

        auto freqParameter = std::make_unique<juce::AudioParameterFloat> (juce::ParameterID{FrequalizerAudioProcessor::getFrequencyParamName (i), 1},
...
//                                                                    juce::NormalisableRange<float> {0.1f, 10.0f, 1.0f, std::log (0.5f) / std::log (0.9f / 9.9f)},
        ...
        auto gainParameter = std::make_unique<juce::AudioParameterFloat> (juce::ParameterID{FrequalizerAudioProcessor::getGainParamName (i),1},
                                                                    prefix + TRANS ("Gain"),
        ...
        auto actvParameter = std::make_unique<juce::AudioParameterBool> (juce::ParameterID{FrequalizerAudioProcessor::getActiveParamName (i),1},
                                                                   prefix + TRANS ("Active"),
                                                                   defaults [i].active,
                                                                   juce::String(),
...
void FrequalizerAudioProcessor::loadHeadphoneSetting(int newValue)
{
    ...
    auto headphoneSetting = headphoneSettings.getProperty(headphoneName, juce::var());
    ...
    for(int i=1; i<=10; i++)
    {
        ...
        auto parameterName = juce::String(i) + "-type";
        ...
        parameterName = juce::String(i) + "-frequency";
        ...
        parameterName = juce::String(i) + "-gain";
        parameterValue = filterSettings.getProperty("Gain", 0.0);
        parameterValue = juce::Decibels::decibelsToGain(parameterValue);
        ...
        parameterName = juce::String(i) + "-quality";
...
{
    auto jsonFile = juce::File::getSpecialLocation(juce::File::SpecialLocationType::userApplicationDataDirectory);
    jsonFile = jsonFile.getChildFile("Hequaliser/headphoneNames.txt");
    
    juce::var json;
    
    juce::String dummySetting = R"(
        {
           "ONE":{
              "Preamp":"-7.0",
              "1":{
...
{
    juce::StringArray headphoneNames;
